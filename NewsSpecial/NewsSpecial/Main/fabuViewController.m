...
- (void)done{
    ...
    if([_textField.text isEqualToString:@""]) {
        [self.view showLoadingMeg:@"请输入视频标题!" time:1];
      
        return;
    }
    if (self.filevideoUrl == nil) {
         [self.view showLoadingMeg:@"请选择需要上传的视频" time:1];
        return;
    }
...
- (void)timerGo{
    ...
    NSLog(@"%f",_count);
    if (_count >= 1) {
        [self.view showLoadingMeg:@"上传成功!我们会对该视频进行审核,审核通过后可在小视频中进行观看" time:3];
       ...
       // [SVProgressHUD showProgress:_count status:@"正在上传..."];
        [self.view showLoadingMeg:@"正在上传..."];
     //   [self.view showLoadingMeg:@"正在上传..." time:1];
     //   [SVProgressHUD showWithStatus:@"正在上传..."];
    }
